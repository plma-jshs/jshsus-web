...
describe('HomeController', () => {
  it('returns the unified dashboard contract with recent free-board posts', async () => {
    const content = {
      listDashboardNotices: vi.fn().mockResolvedValue([]),
      listDashboardLostItems: vi.fn().mockResolvedValue([]),
      listBoardPosts: vi.fn().mockResolvedValue([{ id: 7, title: '최근 글' }]),
    ...
    const controller = new HomeController(content, petitions, schoolData);

    const result = await controller.dashboard();

    expect(content.listBoardPosts).toHaveBeenCalledWith('free', 5);
