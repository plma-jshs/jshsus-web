...
| `S3_PUBLIC_BASE_URL`             | 프로필 이미지 등 공개 예외 URL 기준 | 게시글 파일은 API redirect 사용 |
| `S3_ENDPOINT`                    | S3 호환 스토리지 endpoint           | AWS S3면 선택                   |
| `S3_FORCE_PATH_STYLE`            | path-style 필요 여부                | MinIO 등                        |
| `FILE_PRESIGNED_URL_TTL_SECONDS` | 게시글 파일 presigned URL TTL       | 예: `300`                       |
...
# 프로필 이미지 등 공개 예외 URL을 구성할 때만 사용합니다.
S3_PUBLIC_BASE_URL=https://cdn.jshsus.kr
S3_ENDPOINT=
S3_FORCE_PATH_STYLE=false
FILE_PRESIGNED_URL_TTL_SECONDS=300
