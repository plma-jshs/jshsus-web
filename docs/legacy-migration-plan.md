...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `iam`         | `id`, `stuid`, `name`, `password`, `restricted`, `jshsus`                                                                                  | `users`, `auth_accounts`  | `stuid`를 identity join key로 사용한다. 기존 IAM/JSHSUS id는 신규 스키마에 보존하지 않고, 로그인 연결은 `auth_accounts`의 provider link로만 관리한다.        |
